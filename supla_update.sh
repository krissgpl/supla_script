...
then
	...
	dialog --clear --yesno "Znalazlem w QNAP signed:   $PLIK  $PLIK2	Czy skopiowac ?" 10 53
		YOUR_CHOOSE=$?;
		if [ "$YOUR_CHOOSE" == 0 ];
		then
	...
	dialog --clear --msgbox "Nie nalazlem w QNAP signed:   $PLIK  $PLIK2	Zapomniales skompilowac !" 10 53
	exit
